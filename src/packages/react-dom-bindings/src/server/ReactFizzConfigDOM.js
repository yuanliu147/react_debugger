...
export function writeCompletedRoot(destination, renderState) {
  ...
  if (typeof href === "string" && href) {
    ...
    if (!resumableState.connectResources[bucket].hasOwnProperty(key)) {
      ...
      if (
        ...
        const resource = [];
        pushLinkImpl(resource, { rel: "preconnect", href, crossOrigin });
        renderState.preconnects.add(resource);
      }
    }
    flushResources(request);
...
export function hoistResources(renderState, source) {
